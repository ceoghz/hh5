<script>
    ...
    function getLaiketuiNoRegisterLoginInfo(store_type) {
        uni.request({
            ...
            success: function(res) {
                let register = res.data.is_register || 1
                //乐活精选免密码登陆配置
     ...
     * 根路径
     */
    const LKT_ROOT_URL = 'https://mall.lhsc99.cn'
     ...
     * 带版本号的路径
     */
    const LKT_ROOT_VERSION_URL = LKT_ROOT_URL + '/' + VERSION + '/'

    /**
     *    乐活精选路径
     */
    const LKT_API_URL = LKT_ROOT_URL + '/' + VERSION + '/index.php?store_id=' + LKT_STORE_ID + '&'

    /**
     * 乐活精选H5默认路径
     */
    const LKT_H5_DEFURL = LKT_ROOT_URL + '/' + VERSION + '/h5/#/'
                ...
                success: function (res) {
                    ...
                    if (code == 200 && res.data.login_status == 1) { //登录未失效的用户
                        ...
                        if (me._back1 && typeof (me._back1)) {
							...
							let msg = me.language.laiketuiCommon.noLogin
    ...
    function ttIOSCantVisualpay () {
        ...
        if (info.platform == 'ios') {
            let msg = 'ios暂不支持虚拟支付'
            if(uni.getStorageSync('language') == 'en'){
            	msg = 'Ios does not support virtual payment yet'
            }
