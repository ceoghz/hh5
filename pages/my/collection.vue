	...
	<view class="collection">
		...
		<view class="collection-content">
			<scroll-view class="scroll-box" scroll-y='true' style="height: 100%;" >
				<view class="list-item" @click="jumpDetail(item.p_id)"  v-for="(item,index) in pro_list" :key="index">
						<radio @click.stop="radioClick(item.id)" v-if="is_editor" class="radio" color="#FD8A03" value="r1" :checked="item.isChecked" />
						<image class="list-item-c" :src="item.imgurl" mode=""></image>
						<view class="list-item-r d-felx flex-column ">
							<view class="list-item-r-1">{{item.product_title}}</view>
							...
							<view class="list-item-r-3">
								...
								<view class="similar" @click.stop="jumpSimilar(item.p_id)">
									找相似
								</view>
							</view>
						</view>
