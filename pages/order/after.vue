<template>
	<view class="after">
			...
			<view class="after-content-main">
				<view class="">
					...
					<view class="d-flex a-center" @click="togglePopup('bottom', 'popup')">
						<view style="color: #999999;">{{kdm}}</view>
		...
		<view class="address-footer" >
			<view class="address-footer-main" @click="submitInfo">
			...
			<view class="popup-content ">
				...
				<view class="popup-content-main">
					<scroll-view class="scrollView" style="height: 100%;" scroll-y="true" >
						<view class="scrollView-item d-flex j-sb a-center" @click="abb(item.kuaidi_name)"  v-for="(item,index) in addressInfo.express" :key = "index">
							 <view  class="item-name">{{item.kuaidi_name}}</view>
						</view>
					</scroll-view>
				</view>
...
<script>
	...
	export default{
		...
		methods:{
			...
			//提交寄回信息
			submitInfo(){
				if(this.kdm==='请输入快递名称'&&this.courier_num ===''&&this.courier_name === ''&&this.courier_phone===''){
					uni.showToast({
						title:'请填写完整信息',
						icon:'none'
					})
				...
				this.$http.post("",{
					...
					action:'order',
					app:'back_send',
					kdcode:this.courier_num,
					kdname:this.courierming,
					lxdh:this.courier_phone,
					lxr:this.courier_name,
					id:this.productinfo.return_order_id
				}).then(res=>{
					console.log(res)
					if(res.data.code === 200){
						uni.showToast({
